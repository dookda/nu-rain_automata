		...
		foreach($dat as $x => $x_value) {
			...
			if($field==$x){
				if(is_numeric($x_value)){
					$sql = "update rain_auto set ".$x." = ".$x_value." where station_id='".$sta_val."' and dcode='".$dcode."'";
				}else{				
					$sql = "update rain_auto set ".$x." = '".$x_value."' where station_id='".$sta_val."' and dcode='".$dcode."'";				
				}			
				echo $sql;
...
$fields = array('location','latitude','longitude','elevation','weather','temp_c','tempmax_c',
				...
				'solar_radiation_min','rain_1hr','rain_3hr','rain_24hr','rain_72hr','rain_gmt');
...
foreach($stas as $sta => $sta_val){				
	...
	$sql = "insert into rain_auto (station_id,tstamp,dcode) values ('".$sta_val."',now()::timestamp with time zone at time zone 'Asia/Bangkok','".$dcode."')";
	pg_query($sql);
